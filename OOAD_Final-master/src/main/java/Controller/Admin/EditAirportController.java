...
@WebServlet(urlPatterns = {"/admin/edit-airport"})
public class EditAirportController extends HttpServlet {
			...
			for (FileItem item : items) {
				if (item.getFieldName().equals("idAP")) {
					...
					if (item.getSize() > 0) {
						...
						airport.setImagePlace(null);
					}
				}
			}
			if (airport.getImagePlace() != null) {
				apService.edit(airport);
			} else {
				apService.editNotImage(airport);
			}
