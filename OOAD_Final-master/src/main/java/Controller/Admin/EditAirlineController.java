...
@WebServlet(urlPatterns = {"/admin/edit-airline"})
public class EditAirlineController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			for (FileItem item : items) {
				if (item.getFieldName().equals("nameAL")) {
				...
				} else if (item.getFieldName().equals("idAL")) {
					airline.setIdAL(item.getString("UTF-8"));
					...
					if (item.getSize() > 0) {
						...
						airline.setLogoAL(null);
					}
				}
			}
			if (airline.getLogoAL() != null) {
				alService.edit(airline);
			} else {
				alService.editNotImage(airline);
			}
